# Raasta — Claude Code Instructions

## Project Identity

**Raasta** (Urdu/Hindi: راستہ — path/way/route) — Navigation and pathfinding engine for the AGNOS ecosystem

- **Type**: Flat library crate
- **License**: GPL-3.0-only
- **MSRV**: 1.89
- **Version**: SemVer 1.0.0
- **Tests**: 502 (494 unit + 6 integration + 2 doc)

## Consumers

kiran (game engine — NPC navigation, crowd movement), joshua (simulation — agent pathfinding), impetus (physics — collision-aware steering)

## Dependency Stack

```
raasta (navigation/pathfinding)
  └── hisab (math — Vec2, Vec3, geometry, spatial structures)
```
...
- **Own the stack.** Use hisab types (Vec2, Vec3), not raw glam re-exports.
- **No magic.** Every operation is measurable, auditable, traceable.
- **`#[non_exhaustive]`** on all public enums.
- **`#[must_use]`** on all pure functions.
- **`#[inline]`** on hot-path functions (A* inner loop, steering tick, grid lookup).
- **`write!` over `format!`** — avoid temporary allocations.
- **Vec arena over HashMap** — grid cells are indexed, not hashed.
